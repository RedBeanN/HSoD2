  ...
  new Vue({
    ...
    data: {
      ...
      settings: {
        legendPosition: 'top',
        loading: false,
        colors: ['#fce762','#99c1e3', '#343ef3', '#c04d51'],
      },
    ...
    methods: {
      async loadTop1kData () {
        const { data } = await axios.get(`https://api-1256168079.cos.ap-chengdu.myqcloud.com/faction/20251/data.json`);
        const rows = {};
        const columns = ['时间']
        ...
        rowsArr.sort((a, b) => {
          return new Date('2025-' + a['时间']) - new Date('2025-' + b['时间']) < 0 ? -1 : 1;
        });
