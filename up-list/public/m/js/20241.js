((Vue, axios) => {
  // const columns = ['时间', '艾尔瓦加尔', '祈', '诺登斯', '伊登', '紬', '樱华'];
  const columns = ['时间', '索尔', '密藏', '提尔', '芬里尔', '怠惰', '寄生'];
  ...
  new Vue({
    ...
    data: {
      ...
      settings: {
        legendPosition: 'top',
        loading: false,
        colors: ['#66ccff','#3a3a4a', '#ee82ee', '#dd3333', '#ff69b4', '#ff9700'],
      },
    ...
    methods: {
      async loadTop1kData () {
        const { data } = await axios.get(`https://api-1256168079.cos.ap-chengdu.myqcloud.com/faction/20241/data.json`);
        const rows = {};
